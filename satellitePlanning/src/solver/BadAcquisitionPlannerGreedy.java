	...
	private class SatellitePlan {
		...
		public boolean isFeasible(){
			...
			if(planningProblem.horizonStart > prevAcqWindow.latestStart)
				return false;		
			double startTime = Math.max(planningProblem.horizonStart,prevAcqWindow.earliestStart); // l acquitision windows va conmmencer a earliest time seulement quand il est plus grand que horizon start
			...
			for(int i=1;i<acqWindows.size();i++){
				...
				startTime = Math.max(prevEndTime+rollAngleTransitionTime,acqWindow.earliestStart); //l acquitision windows va conmmencer a earliest time seulement quand il est plus grand que la fin de l acquisition anterieure plus le temps de transition
