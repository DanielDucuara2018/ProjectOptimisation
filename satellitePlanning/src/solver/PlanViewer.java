...
public class PlanViewer {
	...
	public void show(PlanningProblem pb){
		...
		List<CandidateAcquisition> seletedAcquisitions = new ArrayList<CandidateAcquisition>();
		for(CandidateAcquisition a : plan.plannedAcquisitions){
			String name = a.name;
			if(a.selectedAcquisitionWindow.satellite == satellite)
				seletedAcquisitions.add(a);
				satelliteTask.addSubtask(new Task("ACQ_"+name, getDate(a.selectedAcquisitionStartTime), getDate(a.selectedAcquisitionEndTime)));			
		}
		Collections.sort(seletedAcquisitions, new Comparator<CandidateAcquisition>(){
			@Override
		...
		for (CandidateAcquisition a : seletedAcquisitions) {
			if(lastAcq == null || 
